...
function MudarPosicao ()
{
	...
	if(Physics.Raycast(ray, hit) && Pause.pause == false)
	{
...
function Update ()
{
	...
	if(Cronometro >= Intervalo)
	{
		...
		if(sorteioObjeto == 0){
			Debug.Log("Não saiu");
			Instantiate(objetos[cor.valor_Aleatorio2 + Random.Range(1,4)], referenciaLancamento.transform.position, Quaternion.identity);
			Cronometro = 0;
			Criar = true;
		}else
		{
			Instantiate(objetos[cor.valor_Aleatorio2], referenciaLancamento.transform.position, Quaternion.identity);
			Cronometro = 0;
			Criar = true;
		}
	}
